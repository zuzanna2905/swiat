...
static void InitConio2(Conio2ThreadData *data) {
	...
	UpdateWScroll();
	//	SetConsoleOutputCP();
	//SetConsoleMode(data->output, ENABLE_PROCESSED_OUTPUT | ENABLE_WRAP_AT_EOL_OUTPUT);
};
...
void clrscr() {
	...
	for (int i = 0; i < thData.width * thData.height; i++) {
	...
	trg.Right = 79; trg.Bottom = thData.height - 1;
...
int puttext(int left, int top, int right, int bottom, void *source) {
	...
	trg.Left = left - 1; trg.Top = top - 1;
	trg.Right = right - 1; trg.Bottom = bottom - 1;
	WriteConsoleOutput(buf, buf, size, pos, &trg);
	// TODO: check return values
	return 1;
};
...
#ifdef _MSC_VER
static int Conio2_Init(void) {
	InitConio2(&thData);
	return 0;
};

static int Conio2_Exit(void) {
	ExitConio2(&thData);
	return 0;
};
...
static cb *autoexit[] = { Conio2_Exit };
